...
exports.getInfo = async (id, options) => {
  ...
  try {
    ...
    if (options.playerClients.includes("TV")) {
      funcs.push(fetchTvPlayer(id, html5player, options));
    }
...
const fetchTvPlayer = async (videoId, html5Player, options) => {
  const payload = {
    context: TV_CONTEXT,
    videoId,
  ...
  return await playerAPI(videoId, payload, TV_CONTEXT.client.userAgent, options);
}
